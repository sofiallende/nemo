   ...
   SUBROUTINE sbc_rnf( kt )
      ...
      IF( MOD( kt - 1, nn_fsbc ) == 0 ) THEN
         ...
         IF( .NOT. l_rnfcpl ) THEN
             ...
             IF( ln_rnf_icb ) THEN
                ! ELIC change
                !   - the iceberg melt flux is associated with a loss of heat by the ocean
                !   - to avoid cooling the ocean below its freezing point, we block the iceberg
                !     melt flux when the surface of the ocean is already close to its freezing
                !     point
                !
                CALL eos_fzp( sss_m(:,:), ztfrz(:,:) )
                WHERE( sst_m(:,:) > ztfrz(:,:) + 0.15_wp )
...
902   IF( ios >  0 )   CALL ctl_nam ( ios , 'namsbc_rnf in configuration namelist' )
      ...
      ELSE IF( ln_rnf_depth_ini ) THEN           ! runoffs applied at the surface
         ...
         !
         ! ELIC change
         !   - the iceberg flux should be taken into account to compute the depth up
         !     to which the runoff must be distributed vertically
         !   - the implementation below is not perfect: it selects the maximum runoff
         !     or iceberg flux (over the records) instead of selecting the maximum
         !     (over the records) of the sum of the runoff and the iceberg flux
         !   - since the runoff and the iceberg flux are often not colocated, this is
         !     not a severe problem
         !   - in addition, since the runoff and iceberg flux could have different
         !     number of records, this would be difficult to improve
         !
         IF( ln_rnf_icb ) THEN
            CALL iom_open( TRIM( sn_i_rnf%clname ), inum )    !  open iceberg flux file
            nbrec = iom_getszuld( inum )
            DO jm = 1, nbrec
               CALL iom_get( inum, jpdom_global, TRIM( sn_i_rnf%clvar ), zrnfcl(:,:,2), jm )   ! read the value in 2
               zrnfcl(:,:,1) = MAXVAL( zrnfcl(:,:,:), DIM=3 )                                  ! store the maximum value in time in 1
            END DO
            CALL iom_close( inum )
         ENDIF
